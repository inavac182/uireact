...
> Support for `validator.ruler()` is deprecated and will be REMOVED in the next major version. For now you can still ue it at your own risk. Plan to migrate over to `validator.field()` to prepare for next version.

#### What is changing?

- So, The `validator.field()` will expose a few more support functions that will be used from `EzForm` component to automatically generate forms from a schema. 
- The `rule().type()` function is removed on `field()` given that now the type will be set up as a parameter of the `field()` fn. For instnace: `validator.field('email')`

More information will be coming as we have progress on the EzForm and UiValidator upgrades.
...
const schema = {
  firstName: validator.field('string').isRequired('First Name is required').length(0, 10, 'First name is not valid'),

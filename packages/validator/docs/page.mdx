  ...
  <table border={1}>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          <UiList>
            <li> ❗️ value: string | number</li>
            <li> ❓ errorMessage</li>
            <li>DEFAULT: "The value is not equal to: (value)"</li>
          </UiList>
        </td>
        <td>
          <UiList>
            <li>✅ When the passed value matches the expected value</li>
            <li>❌ Everything else</li>
          </UiList>
        </td>
      </tr>
...
There are use cases when depending in some values we need to mutate the field type of a field, for this we have the `mutate()` function.
...
That schema will validate a phone is passed when the method selected is "phone", otherwise it will validate a valid email is passed.

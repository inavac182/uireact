...
import { UiReactViewPulse } from '@uireact/framer-animations';
...
In this example we are using an animation object from package @uireact/framer-animation throught the prop `motion`. 
There are several built-in animations to choose from, you can visit the [framer animations doc](./docs/framer-animations) to learn more 
about all the animations.

```jsx live scope={{UiCard, UiHeading, UiText, UiReactViewPulse}}
...
The outlined card will stylized some content such as texts, although if you use any text component

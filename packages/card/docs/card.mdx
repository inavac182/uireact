---
name: UiCard
menu: Components
---

import { Props } from 'docz';

import { UiText, UiHeading } from '@uireact/text';

import { UiSpacing, ColorTokens } from '@uireact/foundation';

import Playground from '../../../src/Playground';
import { UiCard } from '../src/';
import * as packageJson from '../package.json';

# UiCard

<sup>
  <a href="https://github.com/inavac182/ui-react/blob/main/packages/card/" target="_blank">
    v{packageJson.version}
  </a>
</sup>

> Let's get that text wrapped

## Installation

> npm i @uireact/foundation @uireact/card

## Usage

### UiCard

<Playground>
  <UiCard>
    <UiHeading>Card header</UiHeading>
    <UiText>This is the content</UiText>
    <UiText size="xsmall">This is the footer</UiText>
  </UiCard>
</Playground>

### Card as a link

<Playground>
  <UiCard link="https://www.uireact.io" bordered>
    <UiText>Open UiReact docs</UiText>
  </UiCard>
</Playground>

### UiCard with click handler

<Playground>
  <UiCard clickHandler={(identifier) => console.log(`Card ${identifier} clicked`)} identifier="My-ID" squared>
    <UiHeading>Click me!</UiHeading>
  </UiCard>
</Playground>

### UiCard with image content

<Playground>
  <UiCard noPadding>
    <img
      src="https://upload.wikimedia.org/wikipedia/commons/e/e1/Colima_360.jpg"
      style={{ width: '100%', borderRadius: '3px 3px 0px 0px' }}
    />
    <UiSpacing padding={{ all: 'five' }}>
      <UiHeading>Card header</UiHeading>
      <UiText>This is the content</UiText>
      <UiText size="xsmall">This is the footer</UiText>
    </UiSpacing>
  </UiCard>
</Playground>

### UiCard with weight

<Playground>
  <UiSpacing margin={{ block: 'three' }}>
    <UiCard weight={ColorTokens.token_10}>
      <UiHeading>Card header</UiHeading>
      <UiText>This is the content</UiText>
      <UiText size="xsmall">This is the footer</UiText>
    </UiCard>
  </UiSpacing>
  <UiSpacing margin={{ block: 'three' }}>
    <UiCard weight={ColorTokens.token_50}>
      <UiHeading>Card header</UiHeading>
      <UiText>This is the content</UiText>
      <UiText size="xsmall">This is the footer</UiText>
    </UiCard>
  </UiSpacing>
  <UiSpacing margin={{ block: 'three' }}>
    <UiCard weight={ColorTokens.token_100}>
      <UiHeading>Card header</UiHeading>
      <UiText>This is the content</UiText>
      <UiText size="xsmall">This is the footer</UiText>
    </UiCard>
  </UiSpacing>
  <UiSpacing margin={{ block: 'three' }}>
    <UiCard weight={ColorTokens.token_150}>
      <UiHeading>Card header</UiHeading>
      <UiText>This is the content</UiText>
      <UiText size="xsmall">This is the footer</UiText>
    </UiCard>
  </UiSpacing>
  <UiSpacing margin={{ block: 'three' }}>
    <UiCard weight={ColorTokens.token_200}>
      <UiHeading>Card header</UiHeading>
      <UiText>This is the content</UiText>
      <UiText size="xsmall">This is the footer</UiText>
    </UiCard>
  </UiSpacing>
</Playground>

### UiCard with state

<Playground>
  <UiSpacing margin={{ block: 'three' }}>
    <UiCard weight={ColorTokens.token_10} state="POSITIVE">
      <UiHeading>Card header</UiHeading>
      <UiText>This is the content</UiText>
      <UiText size="xsmall">This is the footer</UiText>
    </UiCard>
  </UiSpacing>
  <UiSpacing margin={{ block: 'three' }}>
    <UiCard weight={ColorTokens.token_50} state="NEGATIVE">
      <UiHeading>Card header</UiHeading>
      <UiText>This is the content</UiText>
      <UiText size="xsmall">This is the footer</UiText>
    </UiCard>
  </UiSpacing>
  <UiSpacing margin={{ block: 'three' }}>
    <UiCard weight={ColorTokens.token_100} state="ERROR">
      <UiHeading>Card header</UiHeading>
      <UiText>This is the content</UiText>
      <UiText size="xsmall">This is the footer</UiText>
    </UiCard>
  </UiSpacing>
</Playground>

## Props

<Props of={UiCard} />

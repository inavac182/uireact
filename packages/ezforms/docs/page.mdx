...
By default the form buttons render inline, although you can pass `buttonAlignemnt` prop as `stacked` and the buttons are going to render stacked:

```jsx live scope={{UiEzForm, UiValidator}}
  <UiEzForm 
    schema={{
      name: new UiValidator().field("text").ezMetada({ label: "Name:" })
    }}

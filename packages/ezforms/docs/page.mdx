...
Using the `mutate()` function we can change the a field's type based on the validations that you set up. For instance in this example we have a field that is a select element but changes to text element when the the `transport` field changes its values.

...
Using the `mutate()` function we can change the a field's type based on the validations that you set up. For instance in this example we have a field that is a select element but changes to text element when the `country` field changes to another value1.
  ...
  <UiEzForm 
    schema={{
      ...
      state: new UiValidator() 
        ...
        .run(new UiValidator().is().present("State is required").oneOf([{ label: 'Colima', value: 1 }, { label: 'Jalisco', value: 2 }]))

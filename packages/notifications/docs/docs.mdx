...
1. Render the component `<UiNotifications />` in next to the `<UiView />`.
...
The passed notification needs to follow this type:
...
Make sure you make sure the [UiIcons](./packages-icons-docs-docs) docs to check the available icons.

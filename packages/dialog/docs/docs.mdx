...
type is exported from this package and is used to type the controller object that has to be set into the dialogs controller
context. This controller will keep the state of all opened dialogs and will provide your own logic for handling the state
of dialogs. E.g. Some apps prefer to keep the dialogs as part of the navigation, other apps just like to keep a react state,
like so, you might have a different use case, so this is your change to make your configure you own controller.

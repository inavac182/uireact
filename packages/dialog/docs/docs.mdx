...
In order for this component to work propertly, there are a few configs to set up:
...
type is exported from this package and should be used to type the controller object so it matches the expectation of the dialogs context.
This controller will keep the state management of all opened dialogs in the application and you can provide your own logic for this.
E.g. Some apps prefer to have the dialogs as part of the navigation, so they need to add a parameter to the URL, other apps just use a react state,
so depending in your use case, this is your chance to make your own implementation for this controller.
...
Once the controller is created and the context is setted up we can go ahead and use the `useDialog` hook to instantiate a dialog.
...
You can see a live example of this in this [DialogsExample](https://github.com/inavac182/uireact/blob/main/packages/dialog/docs/utils/DialogsExample.tsx#L15), this component is used to render all the examples in this doc page.
